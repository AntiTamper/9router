...
describe("Kimi Code API key provider", () => {
  ...
  it("gives Kimi Responses compact requests a longer first-byte timeout", () => {
    ...
    expect(resolveFetchConnectTimeoutMs({ provider: "kimi", sourceFormat: FORMATS.OPENAI })).toBe(60000);
    expect(resolveFetchConnectTimeoutMs({ provider: "openai", sourceFormat: FORMATS.OPENAI_RESPONSES })).toBe(60000);
  });
