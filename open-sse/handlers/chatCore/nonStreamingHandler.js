...
export function translateNonStreamingResponse(responseBody, targetFormat, sourceFormat) {
  ...
  if (targetFormat === FORMATS.CLAUDE) {
    // Always translate a Claude-format body to OpenAI, even if `content` is missing/null
    // (e.g. a thinking model with max_tokens:1 spends the budget on thinking and returns
    // `content: null`). Returning the raw Claude body would leave the OpenAI client without a
    // `choices` array and surface as a UI/test error. Only bail when content is a non-array truthy.
