...
export async function POST(request) {
  try {
    const { providerAlias, id, type, name, maxInputTokens, maxOutputTokens, contextWindow } = await request.json();
    if (!providerAlias || !id) {
      return NextResponse.json({ error: "providerAlias and id required" }, { status: 400 });
    }
    const added = await addCustomModel({ providerAlias, id, type: type || "llm", name, maxInputTokens, maxOutputTokens, contextWindow });
