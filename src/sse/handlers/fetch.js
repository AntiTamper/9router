...
export async function handleFetch(request) {
  ...
  // SSRF guard: reject internal/private/metadata targets (GHSA-qj3v-64wj-q825)
  try {
    assertPublicUrl(targetUrl);
  } catch (e) {
    log.warn("FETCH", "Blocked SSRF target", { url: targetUrl, reason: e.message });
    return errorResponse(HTTP_STATUS.BAD_REQUEST, e.message || "Blocked URL");
  }
