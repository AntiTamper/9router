...
// Models that must NEVER be re-routed -> always passthrough to the real upstream, even when the
// tool's other models are mapped. Antigravity tab-autocomplete (tab_jump_flash_lite_preview /
// tab_flash_lite_preview, requestType tab/tab_jump) is latency-critical inline completion; routing
// it through 9Router makes typing laggy and burns provider quota per keystroke. Without this guard
// the broad `flash` pattern in MODEL_PATTERNS hijacks them onto the flash-agent slot.
const MODEL_NO_MAP = {
  antigravity: [/^tab[_-]/i],
};
